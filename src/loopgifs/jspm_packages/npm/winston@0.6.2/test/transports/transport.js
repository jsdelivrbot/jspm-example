...
module.exports = function(transport, options) {
  var logger = transport instanceof winston.Logger ? transport : new winston.Logger({transports: [new transport(options)]});
  ...
  var out = {
    'topic': logger,
    'when passed valid options': {'should have the proper methods defined': function() {
      ...
      'using no options': {
        ...
        'should stream logs': function(err, results) {
          ...
          results.forEach(function(log) {
            assert.ok(log.message.indexOf('hello world') === 0 || log.message.indexOf('test message') === 0);
          });
