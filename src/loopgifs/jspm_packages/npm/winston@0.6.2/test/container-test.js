...
vows.describe('winston/container').addBatch({
  ...
  "An instance of winston.Container with explicit transports": {
    topic: function() {
      this.port = 9412;
      this.transports = [new winston.transports.Webhook({port: this.port})];
      this.container = new winston.Container({transports: this.transports});
      return null;
    },
    "the get() method": {
      topic: function(container) {
      ...
      "a second call to get()": {"should respond with the same transport object": function() {
