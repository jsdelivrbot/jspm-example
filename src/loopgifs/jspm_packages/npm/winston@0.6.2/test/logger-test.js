...
vows.describe('winton/logger').addBatch({"An instance of winston.Logger": {
    ...
    "the add() with an unsupported transport": {"should throw an error": function() {
        assert.throws(function() {
          logger.add('unsupported');
        }, Error);
      }}
  }}).addBatch({"An instance of winston.Logger with no transports": {
    ...
    "the add() method with a supported transport": {
      ...
      "and adding an additional transport": {
        topic: function(logger) {
          return logger.add(winston.transports.File, {filename: path.join(__dirname, 'fixtures', 'logs', 'testfile2.log')});
        },
  ...
  }}).addBatch({"The winston logger": {
    topic: new (winston.Logger)({transports: [new (winston.transports.Console)(), new (winston.transports.File)({filename: path.join(__dirname, 'fixtures', 'logs', 'filelog.log')})]}),
    "should return have two transports": function(logger) {
      assert.equal(helpers.size(logger.transports), 2);
    },
    "the remove() with an unadded transport": {"should throw an Error": function(logger) {
        assert.throws(function() {
    ...
    topic: new (winston.Logger)({transports: [new (winston.transports.Console)(), new (winston.transports.File)({filename: path.join(__dirname, 'fixtures', 'logs', 'filelog.log')})]}),
    "the clear() method": {"should remove all transports": function(logger) {
        logger.clear();
        assert.equal(helpers.size(logger.transports), 0);
      }}
  }}).export(module);
