...
vows.describe('prompt/interactive').addBatch({"When using prompt": {
    topic: function() {
    ...
    "the getInput() method": {"when passed a complex property with `hidden: true`": {
        topic: function() {
          winston.info('When prompted, enter: 12345 [backspace] [backspace] [enter]');
          prompt.getInput({
            path: ['password'],
            schema: helpers.schema.properties.password
          }, this.callback);
        },
        ...
        "and then when passed a complex property expecting a number": {
          topic: function() {
            ...
            prompt.getInput({
              path: ['number'],
              schema: helpers.schema.properties.number
            }, this.callback);
          },
